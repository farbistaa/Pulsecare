import { pgTable, varchar, decimal, text, serial, integer, boolean, timestamp, jsonb, json, date, index, numeric } from "drizzle-orm/pg-core";
...
export const users = pgTable("users", {
  ...
  lastDonation: text("last_donation_date"),
  ...
  lastDonationDate: text('last_donation_date_date'),
  gender: text("gender"),
  ...
  lastDonationIdx: index("users_last_donation_date_idx").on(t.lastDonation),
}));

export const emergencyRequests = pgTable("emergency_blood_requests", {
  id: serial("id").primaryKey(),
  ...
  bloodGroupIdx: index("emergency_blood_group_idx").on(t.bloodGroup),
  statusIdx: index("emergency_status_idx").on(t.status),
  requesterIdx: index("emergency_requester_idx").on(t.requesterId),
  emergencyDashboardIdx: index("emergency_dashboard_idx").on(t.bloodGroup, t.status),
}));

export const donations = pgTable("donations", {
  id: serial("id").primaryKey(),
  createdAt: timestamp("created_at").defaultNow(),
  donorId: varchar ("donorId").references(() => users.donorId).notNull(),
  recipientId: varchar("recipientId").references(() => users.donorId).notNull(),
  ...
  donorIdIdx: index("donations_donorId_idx").on(t.donorId),
  bloodGroupIdx: index("donations_blood_group_idx").on(t.bloodGroup),
  donationDateIdx: index("donations_date_idx").on(t.donationDate),
  donationStatsIdx: index("donations_stats_idx").on(t.bloodGroup, t.donationDate),
}));
...
export const messages = pgTable("messages", {
  id: serial("id").primaryKey(),
  senderId: varchar("senderId").references(() => users.donorId).notNull(),
  recipientId: varchar("recipientId").references(() => users.donorId).notNull(),
...
export const testimonials = pgTable("testimonials", {
  id: serial("id").primaryKey(),
  reviewerId: varchar("reviewerId").references(() => users.donorId).notNull(),
  revieweeId: varchar("revieweeId").references(() => users.donorId).notNull(),
...
export const educationHistory = pgTable("education_history", {
  ...
  institutionTypeIdx: index("education_history_type_idx").on(t.institutionType),
}));
...
export const donationHistory = pgTable("donation_history", {
  ...
  donationPicture: text("donation_picture"),
  createdAt: timestamp("created_at").defaultNow(),
}, (t) => ({
  userIdIdx: index("donation_history_user_idx").on(t.userId),
...
export const donationAppointments = pgTable('donation_appointments', {
  ...
  requestId: integer('request_id').references(() => emergencyRequests.id, { onDelete: 'cascade' }),
...
export const bloodDonations = pgTable("blood_donations", {
  ...
  volume: integer("volume").notNull().default(450),
  donationType: text("donation_type").notNull().default("Whole Blood"),
  ...
  processedIdx: index("blood_donations_processed_idx").on(t.processed),
}));
...
export const emergencyBloodRequests = pgTable("emergency_blood_requests", {
  ...
  urgencyLevelIdx: index("emergency_blood_req_urgency_idx").on(t.urgencyLevel),
  emergencyManagementIdx: index("emergency_management_idx").on(t.bloodGroup, t.status, t.urgencyLevel),
  userIdIdx: index("emergency_blood_req_user_idx").on(t.userId),
...
export const insertBloodInventorySchema = createInsertSchema(bloodInventory).omit({ id: true, lastUpdated: true });

export const insertBloodDonationSchema = createInsertSchema(bloodDonations).omit({ id: true, createdAt: true, updatedAt: true });

export const insertBloodIssuanceSchema = createInsertSchema(bloodIssuance).omit({ id: true, createdAt: true });

export const insertEmergencyBloodRequestSchema = createInsertSchema(emergencyBloodRequests).omit({ id: true, createdAt: true, updatedAt: true });

export const insertVerificationRequestSchema = createInsertSchema(verificationRequests).omit({ id: true, createdAt: true });

export const insertReactivationRequestSchema = createInsertSchema(reactivationRequests).omit({ id: true, createdAt: true });
...
export const insertBulkOperationLogSchema = createInsertSchema(bulkOperationsLog).omit({ id: true, createdAt: true });
